...
@login_required
def export():
    sel_cat = request.args.get("category", "全部")
    data = load_dashboard_data(BASE_DIR, sel_cat)
    # data里的category_rows是字典列表，转为csv
    ...
    return Response(
        output,
        mimetype="text/csv",
        headers={"Content-Disposition": f"attachment; filename=品类数据_{sel_cat}.csv"}
    )
